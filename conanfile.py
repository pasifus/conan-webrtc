...
class Webrtc(ConanFile):
    ...
    def source(self):
        ...
        git_depot_tools.clone(self.conan_data["sources"][self.version][0]["url"], self.conan_data["sources"][self.version][0]["branch"])
    ...
    def _gclient_revision(self):
        ...
        stdout = StringIO()
        self.run("git ls-remote %s --heads %s | head -n 1 | cut -f 1" % (self.conan_data["sources"][self.version][1]["url"],
                                                      self.conan_data["sources"][self.version][1]["branch"]),  
                                                      output=stdout)
    ...
    def _gn_args(self):
        ...
        # add args by os
        if self.settings.os == "Windows":
            args.extend(['use_custom_libcxx=false', # 'is_clang=false', 
                         'treat_warnings_as_errors=false']) # , 'visual_studio_version=\\"2019\\" visual_studio_path=\\"C:\\Program Files (x86)\\Microsoft Visual Studio\\2019\\Community\\"'
            ...
            args.extend(['is_clang=true', 'use_custom_libcxx=false', 'enable_dsyms=true', 'ios_enable_code_signing=false'])
        else:
            raise Exception("not valid os: ", self.settings.os)
        return " ".join(args)
    ...
    def package_info(self):
        ...
        if self.settings.os == "Windows":
            ...
            self.cpp_info.system_libs.extend(["secur32", "winmm", "dmoguids", "wmcodecdspuuid", "msdmo", "Strmiids", "Iphlpapi.lib"])
        elif self.settings.os == "Linux":
            self.cpp_info.system_libs = ["dl"]
            self.cpp_info.defines.extend(["WEBRTC_POSIX", "WEBRTC_LINUX"])
            self.cpp_info.system_libs.extend(["Xtst", "Xfixes", "Xrandr", "Xext", "Xcomposite", "Xdamage", "X11", "glib-2.0", "pthread"])
        elif self.settings.os == "Macos":
